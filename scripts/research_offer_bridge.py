...
def run(queue_path=QUEUE,max_games=None,runner=subprocess.run,env=None):
    ...
    status={'phase':'PHASE3_RESEARCH_BRIDGE_V28','runAt':now_iso(),'maxGames':limit,'selected':len(chosen),
...
def main():
    ...
    if missing:
        atomic_json(STATUS,{'phase':'PHASE3_RESEARCH_BRIDGE_V28','runAt':now_iso(),'success':False,'error':'missing_required_secrets','missingSecretNames':missing,'publicationTouched':False})
        return 2
